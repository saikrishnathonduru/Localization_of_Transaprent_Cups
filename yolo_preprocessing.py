################################
# yolo_preprocessing file does the preprocessing work on the image dataset(Both Upright as well as fallen cups)
################################
...
def read_annotations(img_dir):
    ...
    for ann_file_name in sorted(os.listdir(img_dir)):
        ...
        if ext == '.txt':
            ...
            img_file_name = img_dir + ann_file_name  # This is annotation file
            ...
            for line in file:  # Read Keypoints on each image
                vWords = line.split()
                obj = {}
...
class YoloBatchGenerator(Sequence):
    ...
    def aug_image(self, train_instance, jitter):  # (not executed)
        image_name = train_instance['filename']
